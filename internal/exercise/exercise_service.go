...
func (ex ExerciseService) GetScore(ctx *gin.Context) {
	...
	questionAnswer := make(map[int]domain.Answer)
	for _, answer := range answers {
		questionAnswer[answer.QuestionID] = answer
	}

	var score int
	for _, question := range exercise.Question {
		if strings.EqualFold(question.CorrectAnswer, questionAnswer[question.ID].Answer) {
